 ...
 *
 */
public class RenamedMethodsCollector4Training {

	private static final String INPUT_DATA_PATH =  Configuration4Training.COMMIT_DIFF_PATH;
	private static final String OUTPUT_DATA_PATH = Configuration4Training.RENAMED_METHODS_PATH;
	
	public static void main(String[] args) {
		System.out.println("Argus: " + args[0]);
		List<String> projectList = CommitDiffs.readList(Configuration4Training.JAVA_REPO_NAMES_FILE);
		int index;
		try {
		...
		if (!new File(filePath).exists()) {
			File projectPath = new File(Configuration4Training.JAVA_REPOS_PATH + projectName);
