...
//			traverseGitRepos(project, Configuration4InCon.JAVA_REPOS_PATH + project + "/.git");
//		}
//	}
	public static void main(String[] args) {
		String JavaReposPath = args[0];
		String reposList = args[1];
		String JavaRepoNamesFile = JavaReposPath + File.separator + reposList;
		String CommitDiffPath = args[2];
		List<String> projectList = readList(JavaRepoNamesFile);
		for (String project : projectList) {
			traverseGitRepos(project, JavaReposPath + File.separator + project + "/.git",
					CommitDiffPath + File.separator + "Commit_Diffs");
		}
	}
	...
	public static void traverseGitRepos(String projectName, String projectGit, String commitDiffPath) {
		String revisedFilesPath = commitDiffPath + File.separator + projectName + "/revFiles/";
		String previousFilesPath = commitDiffPath + File.separator + projectName + "/prevFiles/";
		FileHelper.createDirectory(revisedFilesPath);
		FileHelper.createDirectory(previousFilesPath);
		FileHelper.deleteDirectory(commitDiffPath + File.separator  + projectName + "/DiffEntries/");
	...
	public static void traverseGitRepos(String projectName, String projectGit) {
		String revisedFilesPath = Configuration4InCon.COMMIT_DIFF_PATH + projectName + "/revFiles/";
		String previousFilesPath = Configuration4InCon.COMMIT_DIFF_PATH + projectName + "/prevFiles/";
		FileHelper.createDirectory(revisedFilesPath);
		FileHelper.createDirectory(previousFilesPath);
		FileHelper.deleteDirectory(Configuration4InCon.COMMIT_DIFF_PATH + projectName + "/DiffEntries/");
		GitRepository gitRepo = new GitRepository(projectGit, revisedFilesPath, previousFilesPath);
		try {
			gitRepo.open();
			List<RevCommit> commits = gitRepo.getAllCommits(false);
