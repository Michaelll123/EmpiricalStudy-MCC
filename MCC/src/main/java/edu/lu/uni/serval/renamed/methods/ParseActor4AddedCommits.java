...
public class ParseActor4AddedCommits extends UntypedActor {
	private static Logger log = LoggerFactory.getLogger(ParseActor4AddedCommits.class);
	...
	int endIndex = 0;
	//lucene-solr
	public ParseActor4AddedCommits(String inputProjectPath, String outputPath) {
		...
		travelRouter = this.getContext().actorOf(new RoundRobinPool(numberOfWorkers)
				.props(ParseWorker4AddedCommits.props(this.outputPath, projectName)), "parse-router");
	}

	public ParseActor4AddedCommits(String inputProjectPath, String outputPath, int startIndex, int endIndex) {
		this.inputProjectPath = inputProjectPath;
		projectName = inputProjectPath.substring(inputProjectPath.lastIndexOf("/") + 1);
		this.outputPath = outputPath;
	...
	public static Props props(final String rootPath, final String outputPath) {
		return Props.create(new Creator<ParseActor4AddedCommits>() {

			private static final long serialVersionUID = -4156981079570315552L;

			@Override
			public ParseActor4AddedCommits create() throws Exception {
				return new ParseActor4AddedCommits(rootPath, outputPath);
			}
		});
	}
	...
	public static Props props(final String rootPath, final String outputPath, final int startIndex, final int endIndex) {
		return Props.create(new Creator<ParseActor4AddedCommits>() {
			...
			@Override
			public ParseActor4AddedCommits create() throws Exception {
				return new ParseActor4AddedCommits(rootPath, outputPath, startIndex, endIndex);
			}
		});
	}
...
//				AddedMethodsFilter.filteroutTyposByParsedMethodNames(outputPath);
	...
	private void mergeData() throws IOException {
		String methodsFile = outputPath + "AddedMethods.txt";
...
//		String newMethodNamesFile = outputPath + "NewMethodNames.txt";
		...
		String path = outputPath + "AddedMethods/";
...
//		StringBuilder newMethodNames = new StringBuilder();
		...
		for (int i = 1; i <= numberOfWorkers; i ++) {
			if (!new File(path + "AddedMethods_" + i + ".txt").exists()) continue;
			FileInputStream fis = new FileInputStream(path + "AddedMethods_" + i + ".txt");
...
//				System.out.println(elements.length);
				if (elements.length == 8) {
					String[] tokens = elements[7].split(" ");
