 ...
 *
 */
public class AddedMethodsFilter {

	private static String rootPath = Configuration4InCon.OUTPUT_PATH;
...
//		filteroutTyposByLevenshteinDistance();
		 ...
		 */
		filteroutTyposByParsedMethodNames(Configuration4InCon.Added_METHODS_PATH);
...
//		FileHelper.outputToFile(rootPath + "Typo/NewNames.txt", typoNewNames, false);
		...
		StringBuilder addedMethodsBuilder = new StringBuilder();
		StringBuilder addMethodTokensSizeBuilder = new StringBuilder();
//		StringBuilder typoMethodsBuilder = new StringBuilder();
//		StringBuilder typoMethodTokensSizeBuilder = new StringBuilder();
		String methodsFile = dataPath + "AddedMethods.txt";
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				addedMethodsBuilder.append(line).append("\n");
				addMethodTokensSizeBuilder.append(tokens.length).append("\n");
		...
		FileHelper.outputToFile(dataPath + "ActualRenamed/MethodTokens.txt", addedMethodsBuilder, false);
		addedMethodsBuilder.setLength(0);
		FileHelper.outputToFile(dataPath + "ActualRenamed/MethodTokensSizes.csv", addMethodTokensSizeBuilder, false);
...
//		FileHelper.outputToFile(dataPath + "Typo/MethodTokensSizes.csv", typoMethodTokensSizeBuilder, false);
		...
		StringBuilder addedMethodBodies = new StringBuilder();
		try {
			...
			while (scanner.hasNextLine()) {
				...
				if ("#METHOD_BODY#========================".equals(line)) {
					if (singleMethod.length() > 0) {
							addedMethodBodies.append(singleMethod);
			...
			FileHelper.outputToFile(dataPath + "ActualRenamed/MethodBodies.txt", addedMethodBodies, false);
