...
public class CodeChangeParser4AddedCommits {
	...
	public void parse(File revFile, File prevFile, File diffentryFile) {
		...
		if (gumTreeResults != null && gumTreeResults.size() != 0) {
			...
			Map<Integer, HierarchicalActionSet> methodInsActionSets = new HashMap<>();
...
//				}
				...
				//if there contains a new declaration of methods.
				if (action.startsWith("INS MethodDeclaration")){
					if (action.contains("@@=CONSTRUCTOR=, MethodName:")) {
						continue;
					}
					methodInsActionSets.put(actionSet.getStartPosition(),actionSet);
				}
			}
			if(methodInsActionSets.size()>0){
				CUCreator cuCreator = new CUCreator();
				CompilationUnit revUnit = cuCreator.createCompilationUnit(revFile);
				for(Map.Entry<Integer,HierarchicalActionSet> entry:methodInsActionSets.entrySet()){
					HierarchicalActionSet actionSet = entry.getValue();
					String methodName = actionSet.getNode().getLabel();
					System.out.println(methodName);
					int semicolonIndex = methodName.indexOf(":");
					int commaIndex = methodName.indexOf(",",semicolonIndex);
					methodName = methodName.substring(semicolonIndex + 1,commaIndex);
...
//					System.out.println(tokens);
					...
					this.addedMethods.add(newCommitId + ":" +filePath + ":" + methodName + ":" + tokens);
				}
...
//		ITree newMethodTree = ((Insert)actionSet.getAction()).getNode();
...
//		if (tokensOfOldMethod.equals(tokensOfNewMethod)) {
			this.methodBodies.append(readMethodBody(sourceCodeFile, cu.getLineNumber(methodTree.getPos()), cu.getLineNumber(methodTree.getPos() + methodTree.getLength())));
